...
- **PDF annotation ink vanished right after drawing (macOS 26).** Freehand pen/highlighter strokes on a PDF showed while drawing, then disappeared on mouse-up. macOS 26's PDFKit no longer renders a programmatically-added ink annotation on screen — no redraw force brings it back (`setNeedsDisplay`, `layoutDocumentView`, even a full document detach/re-attach all fail). The reader now paints the ink itself on a lightweight overlay that re-projects to track the page on scroll/zoom, while still adding the `PDFAnnotation` so the stroke saves into the PDF and shows in Preview / other readers. (The same code rendered fine on older macOS — an OS behavior change, not a code regression.)
- **Hardened the local-asset loader against file exfiltration.** The preview now serves only files under the open project (or the previewed note's own folder); a crafted note can no longer pull arbitrary paths like `~/.ssh/…` through the asset scheme. Image references that legitimately point outside the project are no longer loaded — keep note assets inside the project.
- **Switching projects could drop unsaved writes.** The security-scoped folder access was released before the per-project database finished flushing on close/switch; it's now released a beat later so pending writes land. Local images in folders with spaces now load, and reopening the same project after closing it refreshes the sidebar.
