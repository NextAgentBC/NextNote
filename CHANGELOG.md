...
- **PDF annotation ink vanished right after drawing (macOS 26).** Freehand pen/highlighter strokes on a PDF showed while drawing, then disappeared on mouse-up. The ink `PDFAnnotation` was built with full-page bounds + absolute-coordinate points, a shape macOS 26's PDFKit no longer renders. Annotations now use tight bounds with the path in the annotation's local space, so the ink stays put (and saves into the PDF).
