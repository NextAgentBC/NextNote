...
/// macOS-side native text editor backed by NSTextView in an NSScrollView.
/// Coordinator handles delegate callbacks, snippet insertion (via
/// `insertText` so undo manager picks it up), and a custom drag drop
/// path that turns dropped media files into `![alt](path)` markdown.
struct MacTextEditorView: NSViewRepresentable {
    ...
    class Coordinator: NSObject, NSTextViewDelegate, NSDraggingDestination {
        ...
        /// Library, Finder, and YouTube-download flows all converge on
        /// the same embed syntax.
        ...
        /// Insert snippet.text at the current cursor; position cursor at
        /// snippet.cursorOffset within the inserted text. Uses
        /// NSTextView.insertText which is tracked by the undo manager.
